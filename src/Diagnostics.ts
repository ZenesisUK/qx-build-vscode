 ...
 * - message
 */
const classIssueRegexp = /^([a-z0-9_\$\.]+):\s(\[\d+,\d+\])\s(\[\d+,\d+\])\s([a-z]+):\s(.*)$/gi;
...
export class Diagnostics {
  constructor(
    ...
    this.buildProcess.on("restart", () => this.diagnosticCollection.clear());
  }
  ...
  private parseLogItem(data: string): LogItem<"class" | "project"> | null {
    ...
    try {
      const [, classname, positionStart, positionEnd, level, message] = classIssueRegexp.exec(data) ?? [];
  ...
  private findFilesForClass(classname: string) {
    ...
    for (const sourcePath of this.buildProcess.sourcePaths) {
      const file = path.resolve(this.workspace.uri.fsPath, sourcePath, ...segments) + ".js";
      if (fs.existsSync(file)) files.push(vscode.Uri.parse(file));
    }
    return files;
  }
